          ...
          <div className="glass border border-white/10 rounded-[2.5rem] p-6 overflow-hidden">
            <div className="aspect-[8.5/11] bg-gradient-to-br from-white/5 to-white/10 rounded-[1.5rem] flex items-center justify-center relative overflow-hidden group">
              ...
              <img 
                src="/resume.jpg" 
                alt="Resume"
                className={`w-full h-full object-contain rounded-[1rem] p-4 ${showPlaceholder ? 'hidden' : ''}`}
                onError={() => setShowPlaceholder(true)}
              />
              {showPlaceholder && (
                <div className="text-center space-y-4 z-10">
                  <svg className="mx-auto text-neon-blue opacity-50" width="80" height="80" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                  ...
                  <p className="text-white/40 font-bold">Resume Preview</p>
                  <p className="text-white/20 text-sm">Add resume.jpg in public folder</p>
                </div>
              )}
            </div>
          </div>
